    ...
    public abstract class AdminControllerBase : ControllerBase
    {
        ...
        public override Operater Operater
        {
            get
            {
                return new Operater()
                {
                    Name = this.peAppLoginInfo == null ? "" : this.peAppLoginInfo.LoginName,
                    Token = this.peAppLoginInfo == null ? Guid.Empty : this.peAppLoginInfo.LoginToken,
                    UserId = this.peAppLoginInfo == null ? 0 : this.peAppLoginInfo.UserID,
                    Time = DateTime.Now,
                    IP = Fetch.UserIp
                };
            }
        ...
        /// 登录后用户信息
        /// </summary>
        public virtual peAppLoginInfo peAppLoginInfo
        {
            get
            {
                return UserContext.peAppLoginInfo;
            }
        }
        ...
        public virtual List<EnumBusinessPermission> PermissionList
        {
            get
            {
                ...
                if (this.peAppLoginInfo != null)
                    permissionList = this.peAppLoginInfo.BusinessPermissionList;
        ...
        protected override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if (permissionAttributes != null && attributes.Count() > 0)
            {
                ...
                foreach (var attr in attributes)
                {
                    ...
                    {
                        if (!this.peAppLoginInfo.BusinessPermissionList.Contains(permission))
                        {
                            hasPermission = false;
                            break;
                        }
